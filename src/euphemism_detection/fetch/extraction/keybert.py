...
class KeyBERTFilter(BaseFilter):
    ...
    def get_most_important_ngrams(self, corpus: List[str], top_k: int, stop_words: List[str] = None, max_sum: bool = False, mmr: bool = False):
        ...
        extracted_words = sorted(sum(extracted_words, []), key=lambda x: x[1], reverse=True)[:top_k]
