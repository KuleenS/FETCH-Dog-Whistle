...
class ChromaDB:
    ...
    def __init__(self, persistent_store_path: str):
        ...
        self.collection = self.chroma_client.get_or_create_collection(name="social_media_posts", metadata={"hnsw:space": "cosine"}, persist_directory=persistent_store_path)
    
    def add_to_collection(self, documents: List[str], embeddings: List[List[float]], dogwhistles: List[str], ids: List[str]):
    ...
    def calculate_seed_word_centroid(self, seed_word: str) -> np.ndarray:
        results = self.collection.get(where={"dogwhstile" : seed_word}, include=["embeddings"])
