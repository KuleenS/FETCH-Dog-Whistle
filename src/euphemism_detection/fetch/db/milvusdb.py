...
class MilvusDB:
    ...
    def __init__(self, collection_name: str, dim: int) -> None:
        ...
        embedding = FieldSchema(name="embeddings", dtype=DataType.FLOAT_VECTOR, dim=dim)
    ...
    def load_data(self, input_folder: str):
        ...
        for file_to_load in files_to_load:
            ...
            for document_batch, embedding_batch, dogwhistle_batch in tqdm(
                group_list(documents, embeddings, dogwhistles, 1024)
            ):
                mr = self.post_lookup.insert(
                    [document_batch, dogwhistle_batch, embedding_batch]
                )
    ...
    def get_top_k_documents(
        self, documents_not_to_include: List[int], centroid: List[float], top_k: int
    ) -> List[str]:
