...
class COLBERTReranker:
    ...
    def __init__(self, corpus: List[str], colbert_path: str, min_ranker: bool = False, max_len: int = 256, nbits: int = 2):
        self.collection = Collection(path=corpus)
    ...
    def dense_search(self, searcher, Q: torch.Tensor, k=10, filter_fn=None, pids=None):
        ...
        pids, scores = searcher.ranker.rank(searcher.config, Q, filter_fn=filter_fn, pids=pids)
    ...
    def search_all(self, searcher, queries, k=10, filter_fn=None, full_length_search=False, qid_to_pids=None):
        queries = Queries.cast(queries)
        queries_ = list(queries.values())

        Q = self.encode(queries_, full_length_search=full_length_search)

        return self._search_all_Q(searcher, queries, Q, k, filter_fn=filter_fn, qid_to_pids=qid_to_pids)

    
    def rerank(self, queries: List[str], dogwhistles: List[str], top_k_colbert: int, top_k_count: int):

        queries_dict = dict(zip(range(len(queries)), queries))

        queries = Queries(path=queries_dict)
        ...
        with Run().context(RunConfig(nranks=1, experiment='reranker')):
            ...
            indexer.index(name="dogwhistles", collection=self.collection, overwrite=True)
