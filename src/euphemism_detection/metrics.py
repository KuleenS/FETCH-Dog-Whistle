...
class MetricsReddit:
    def __init__(self, reddit_file: str):
        ...
        self.ngrams = {z : min([len(word_tokenize(y)) for y in list(x) + [z]]) for z,x in zip(self.dogwhistle_data["dog_whistle_root"], self.dogwhistle_data["dog_whistle"])}
    ...
    def measure_precision(self, predicted_dogwhistles: List[str], gold_dogwhistles: List[str]) -> float:
        dogwhistle_map = {x.strip().lower(): self.dogwhistles[x.strip().lower()] for x in gold_dogwhistles}

        predicted_dogwhistles_cleaned = [x.strip().lower() for x in predicted_dogwhistles]
...
class MetricsOther:
    ...
    def __init__(self, dogwhistle_file_path: str) -> None:
        ...
        comparison_set = [x.strip().lower() for x in dogwhistles_df["Dogwhistle"].tolist()]
        
        self.dogwhistle_to_surface = defaultdict(list)

        for i in range(len(dogwhistle_set)):
            self.dogwhistle_to_surface[comparison_set[i]] = set([x.strip().lower() for x in dogwhistle_set[i]] + [comparison_set[i]])
