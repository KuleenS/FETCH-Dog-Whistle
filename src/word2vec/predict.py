...
def find_vocabulary_terms(keywords, model):
    """ """
    ## Initialize Tracker
    ...
    keyword_terms = list(
        zip(
            get_ordered_vocabulary(model),
            list(map(tracker.search, get_ordered_vocabulary(model))),
        )
    )
...
def get_similar(model, word, top_k=10):
    """ """
...
def main(args):
    ...
    term2keywords = find_vocabulary_terms(keywords=keywords, model=model)
    ...
    for round in range(args.n_rounds):
        ...
        ## Run Search
        round_expansion = {
            q: [i[0] for i in get_similar(model, q, top_k=args.top_k)] for q in queries
        }
        ## Track New Terms
        round_new = []
        ## Parse Search Result
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--keywords",
        type=str,
        help="Path to .keywords file containing words to expand.",
    )

    parser.add_argument(
        "--output_dir", type=str, default=None, help="Where to output results."
    )

    parser.add_argument("--n_rounds", type=int, default=1)

    parser.add_argument(
        "--top_k", type=int, help="How many terms to expand within each round."
    )
